function [convged,Corr_valid,cov_Fa,mn_Fa,cov_Xa,mn_Xa,mean_all]=iterLowO_Ref(Nc,mu_vec,sig_vec,tau_vec,rv_vec,sp_vec,Gm,CinMat)
%[convged,Corr_valid,cov_Fa,mn_Fa,cov_Xa,mn_Xa,mean_all]=iterLowO_Ref(Nc,mu_vec,sig_vec,tau_vec,rv_vec,sp_vec,Gm,CinMat);
% complete diff method based on Ref's #2 comment; reduction to 1st order in g
...
sclSig=1; %!!!! changed cf. to iter_method[].m !!!
...
for j=2:Num_it
    ...
    Mnf=diag(sclSig*norm01_pdf'*(Finst_1D.*(normDom_mat*diag(2*tau_vec.*sqrt(var_prev)) ))*d_nrm);
    %Mnf have save sparsity structure as CinMat
...
%         Mfsq(cl_ind(k),rw_ind(k))=Mfsq(rw_ind(k),cl_ind(k)); %make symmetric, this is faster then outside for-loop
        Mnf(rw_ind(k),cl_ind(k))=sclSig*2*tau_vec(cl_ind(k))*sqrt(var_prev(cl_ind(k)))*...
        ...
        Mnf(cl_ind(k),rw_ind(k))=sclSig*2*tau_vec(rw_ind(k))*sqrt(var_prev(rw_ind(k)))*...
            sum(repmat(Finst_1D(:,cl_ind(k)),len_nrm,1).*nrm_varS...
            .*squeeze(norm2D_all(rw_ind(k),cl_ind(k),:)))*d_nrm*d_nrm;
    end
    
    %perform the calculation
    cov_Xa=tauInv_scl.*(diag(sig_vec)*CinMat*diag(sig_vec)+Gm*Mnf+Mnf'*Gm');
