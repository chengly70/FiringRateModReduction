function [convged,Corr_valid,cov_Fa,mn_Fa,cov_Xa,mn_Xa,mean_all]=iter_method_large(Nc,mu_vec,sig_vec,tau_vec,rv_vec,sp_vec,Gm,CinMat)
%[convged,Corr_valid,cov_Fa,mn_Fa,cov_Xa,mn_Xa,mean_all]=iter_method_large(Nc,mu_vec,sig_vec,tau_vec,rv_vec,sp_vec,Gm,CinMat);
%Simulate full network of Nc (input) firing rate network (WC), using Analytic method
% !!! BUT assume that Nc>100 so unwieldy to do entire Cov matrices !!!,
% only use tri-diagonal, band above and below
...
id1=(1:Nc-1)'; %band above diagonal
id2=id1+1;
ind_UpTri=sub2ind([Nc Nc],id1,id2); %indices of band above diagonal
...
if(num_Crs~=0) %only eval if have background correl
    norm2D_all=zeros(num_Crs,len_nrm*len_nrm); %2D matrix, all correl2Dgaussians
    for j=1:num_Crs
        sigTmp=[1 CinMat(rw_ind(j),cl_ind(j));  CinMat(rw_ind(j),cl_ind(j)) 1];
        norm2D_all(j,:)=mvnpdf([nrm_varF nrm_varS],[0 0],sigTmp); %0 mean, unit var, cov=CinMat(j,k)
    end
end
...
cov_Xa=sparse(tauInv_scl.*diag(sig_vec)*CinMat*diag(sig_vec));  %entire cov matrix of Xj (includes var)
corr_Xa=sparse(Nc,Nc); %entire correl matrix of Xj (ones on diagonal)
mn_Fa=zeros(Nc,1);    %mean of F(Xj)
cov_Fa=sparse(Nc,Nc);  %entire cov matrix of Fj (includes var)
...
Mfsq=sparse(Nc,Nc); %matrix for cov of coupling direct terms
Mnf=sparse(Nc,Nc);  %matrix for cov of coupling with noise term
...
for j=2:Num_it
    ...
    for k=1:num_Crs 
        Mfsq(rw_ind(k),cl_ind(k))=...
            ...
            .*(norm2D_all(k,:)'))*d_nrm*d_nrm - F_mn(rw_ind(k))*F_mn(cl_ind(k));
    ...
    var_tmp=diag(cov_Xa);
    cov_tmpT=sparse(triu(cov_Xa)-diag(var_tmp));
...
if(convged==0)
    ...
    %calc the corr(Xj,Xk)
    corr_Xa=sparse(cov_Xa./sqrt(varX_matr));
    ...
    %Recalc numeric 2D-Gauss with FINAL cov/corr, calc all only nonzeros..
    %not saving 2D-Gauss; finish calculation here, cov_Fa off-diagonal
    for k=1:num_Crs 
        sigTmp=[1 corr_Xa(rw_ind(k),cl_ind(k)); corr_Xa(rw_ind(k),cl_ind(k))  1];
        norm2D_sing=mvnpdf([nrm_varF nrm_varS],[0 0],sigTmp); %0 mean, unit var, cov=corr_Xa
        
        cov_Fa(rw_ind(k),cl_ind(k))=...
            sum(repmat(Finst_1D(:,rw_ind(k)),len_nrm,1).*reshape(repmat(Finst_1D(:,cl_ind(k))',len_nrm,1),len_nrm*len_nrm,1)...
            .*norm2D_sing)*d_nrm*d_nrm - mn_Fa(rw_ind(k))*mn_Fa(cl_ind(k));
        
        cov_Fa(cl_ind(k),rw_ind(k))=cov_Fa(rw_ind(k),cl_ind(k)); %make symmetric
    end
